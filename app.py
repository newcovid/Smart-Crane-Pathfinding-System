...
# 智能起重机路径规划系统主入口
#
# 版权与作者信息见 LICENSE。此处不再重复署名与日期——
# 源码头部的日期不会随修改更新，只会变成过期信息。
#
# 本模块负责：
...
def handle_latency_probe(_data=None):
    """空回执，仅用于前端测量往返时延。

    Socket.IO v4 的心跳完全在传输层内部完成，不再向应用层派发 'pong'，
    因此延迟需要由应用自己测。返回值会作为 ack 回传给客户端回调。
    """
    return True
