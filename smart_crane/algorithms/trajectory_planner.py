...
class TrajectoryPlanner:
    ...
    def initialize_planner(self, force_rebuild: bool = False):
        ...
        with self.grid_lock:
            ...
            self.logger.info(f"核心重构完成: 算法类型={algo_type.upper()}")
    ...
    def plan(
        ...
        try:
            with self.grid_lock:
                ...
                stats["grid_meta"] = {
                    "dims": [self.map_mgr.rows, self.map_mgr.cols, self.map_mgr.layers],
                ...
                if needs_escape or not self.core_planner.is_safe(s_node):
                    esc_node = self.safety_guard.smart_escape(
                        s_node, e_node, self.core_planner
                    )
                    if esc_node:
                ...
                if is_fixed_h:
                    # 2.5D 模式：起点 -> 升至巡航高度 -> 巡航路径 -> 降至终点
                    full_path = (
                        [(exact_start[0], exact_start[1], cruise_z)]
                        + path_world
                        + [(exact_end[0], exact_end[1], cruise_z)]
                    )
                else:
                    # 3D 模式：直接连接
                    full_path = [exact_start] + path_world + [exact_end]
                ...
                # Phase 7: Post Processing (后处理优化)
                # 创建一个碰撞检测闭包，用于后续的平滑算法
                checker = self.safety_guard.create_collision_checker(
                    full_path[0], full_path[-1], self.settings
                )

                opt_path = full_path
                for proc in self.post_processors:
                    # 将当前路径流向下一个处理器
                    opt_path = proc.process(opt_path, checker)
                ...
                if is_fixed_h:
                    # 确保垂直起降段的完整性
                    # 如果起点高度与巡航高度不同，补充起点
                    if abs(exact_start[2] - opt_path[0][2]) > 0.01:
                        final_path.append(exact_start)

                    final_path.extend(opt_path)

                    # 补充终点
                    if abs(exact_end[2] - opt_path[-1][2]) > 0.01:
