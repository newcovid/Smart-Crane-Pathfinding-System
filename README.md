...
| 项 | 保证 |
|---|---|
| C-Space 膨胀网格 | **逐格相同**（含 0.5 / 1.0 / 2.0 三种分辨率） |
| D\* Lite 路径 | **逐点相同** |
| A\* 路径 | 代价相同，节点序列可能不同（见下） |

A\* 是唯一不保证逐点一致的部分。两侧的 tie-break 规则本身是一致的
（先比 f，再比坐标字典序），差异来自浮点宽度：相差 1e-8 的两个 f
在 `f32` 下并列、在 `f64` 下不并列，弹出顺序随之改变。两条路径都是最优解，
因此没有把 Rust 改成 `f64` ——那是拿真实性能换一个不影响正确性的性质。
